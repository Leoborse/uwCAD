...
self.addEventListener('install', (event) => {
  event.waitUntil(
    caches.open(cacheInfos.name).then( (cache) => {
      console.log("Opened cache",cacheInfos.name)
...
self.addEventListener('fetch', function(event) {
  ...
  event.respondWith(
    caches.open(cacheInfos.name).then(function(cache) {
      if ( event.request.url.indexOf("/cache.infos") !== -1 ) {
        console.log(event.request.url)
        return new Response(JSON.stringify(cacheInfos), {
      ...
      return cache.match(event.request).then(function(response) {
        ...
        return fetch(event.request.clone()).then(function(response) {
          console.log('  Response for %s from network is: %s, %O',
            event.request.url, response.statusText, response);
...
self.addEventListener('activate', (event) => {
  ...
  cacheWithelist.push(cacheInfos.name)

  event.waitUntil(
    caches.keys().then( (cacheNames) => Promise.all(
      cacheNames.map( (cacheName) => {
        if ( !cacheWithelist.includes(cacheName) ) {
          console.log('Deleting cache %s',cacheName)
